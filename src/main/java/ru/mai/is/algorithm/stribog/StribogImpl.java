...
public class StribogImpl implements Stribog{
    ...
    /**
     * Инициализирует переменные, необходимые для хэширования сообщения
     *
     * @param outputMode Если outputMode равен false (Стрибог-256),
     *                   вектор инициализируется значением 0x01, заполняя все байты значением 0x01.
     *                   Если outputMode равен true (Стрибог-512), вектор инициализируется значением 0x00,
     *                   заполняя все байты нулями.
     * Вектор инициализации h - это вектор состояния (инициализирующий вектор), который используется в функции
     *                   сжатия Стрибог. Он всегда имеет размер 64 байта (512 бит).
     * Счетчик длины сообщения N - используется для хранения общей длины обработанного сообщения в битах.
     *                   Он также имеет размер 64 байта и инициализируется нулями (0x00),
     *                   поскольку в начале ни одного блока не обработано.
     * Сумма всех обработанных блоков сообщения sigma - sigma хранит сумму всех обработанных блоков сообщения,
     *                   используя арифметику по модулю 2^512.
     *                   Этот массив также имеет размер 64 байта и инициализируется нулями (0x00).
     */
    protected void init(boolean outputMode)
    {
        ...
        N = new byte[64]; // счетчик длины сообщения
        Arrays.fill(N, (byte)0x00);
        
        sigma = new byte[64]; // сумма всех обработанных блоков сообщения
    ...
    private byte[] generateHash(byte[] message, boolean outputMode) {
        int length = message.length * 8;  // Длина сообщения в битах
        int inc = 0;  // Счетчик для обработки блоков сообщения

        // Основной цикл для обработки блоков по 512 бит (64 байта)
        while (length >= 512) {
            inc++;
            byte[] tmp = Arrays.copyOfRange(message, message.length - 64 * inc, message.length - (inc - 1) * 64);
            h = compression(N, tmp, h);  // Применение функции сжатия
            N = addModule(N, N512);  // Увеличение счетчика длины на 512 бит
            sigma = addModule(sigma, tmp);  // Обновление суммы всех блоков
            length -= 512;  // Уменьшение длины на 512 бит
        }

        // Оставшаяся часть сообщения, если длина меньше 512 бит
        message = Arrays.copyOf(message, message.length - 64 * inc);

        // Обработка сообщений длиной меньше 512 бит
        if (message.length < 64) {
            byte[] tmpMessage = new byte[64];  // Новый блок на 64 байта
            Arrays.fill(tmpMessage, (byte) 0x00);  // Заполнение нулями
            tmpMessage[64 - message.length - 1] = (byte) 0x01;  // Установка единичного бита в нужной позиции

            // Копирование оставшегося сообщения в конец нового блока
            for (int i = 64 - message.length; i < 64; i++) {
                tmpMessage[i] = message[i + message.length - 64];
            }
            message = tmpMessage;  // Подготовленное сообщение
        }

        // Сжатие последнего блока
        h = compression(N, message, h);

        // Подготовка байтовой строки для длины сообщения
        byte[] NMessage = new byte[64];
        Arrays.fill(NMessage, (byte) 0x00);

        // Представление длины сообщения в виде массива байт
        inc = 0;
        while (length > 0) {
            NMessage[63 - inc] = (byte) (length & 0xff);
            length >>= 8;
        ...
        // Обновление длины и суммы для окончательного сжатия
        N = addModule(N, NMessage);
        sigma = addModule(sigma, message);

        // Финальные сжатия с использованием начального значения N0
        h = compression(N0, N, h);
        h = compression(N0, sigma, h);

        // Возврат окончательного значения хеша
        if (outputMode)
            return h;  // Стрибог-512
        return Arrays.copyOf(h, 32);  // Стрибог-256 (первые 32 байта)
    }
     ...
     * Переставляет элементы массива в соответствие с таблицей
     * @param a блок данных
     * @return 
     */
